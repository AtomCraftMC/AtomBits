package ir.alijk.megacore.config;
...
public abstract  class Configurable {
        ...
        public void create()
        {
                ...
                file = new File(AtomBits.getInstance().getDataFolder(), this.configName);

                // Checking if config exists, if not creating one with default values
                if (!file.exists()) {
                        file.getParentFile().mkdirs();
                        AtomBits.getInstance().saveResource(this.configName, false);
                }
        }
